<template>
    <div @touchend="stopSwipe" ref="menu" :style="{height: (showRecommendations || showWeather) ? '60vh' : (showTimelapse ? '35vh' : '100px')}" class="w-full text-sm bg-white rounded-t-3xl duration-300 ease-in-out overflow-hidden">
        <div @touchstart="startSwipe" class="w-full px-4 flex justify-between items-center h-[100px]">
        ...
        <div class="overflow-y-scroll overflow-x-hidden h-full">
            <Recommendations @goToLocation2="goToLocation2" v-show="showRecommendations" />
